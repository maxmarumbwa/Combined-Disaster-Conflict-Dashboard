# Generated by Django 5.1.1 on 2026-01-20 15:41

import django.db.models.deletion
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('regions', '0006_adm1_shapename2'),
    ]

    operations = [
        migrations.CreateModel(
            name='ConflictEvent',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('country', models.CharField(max_length=100)),
                ('iso3', models.CharField(max_length=3)),
                ('admin2_name', models.CharField(max_length=110)),
                ('admin1_pcode', models.CharField(max_length=20)),
                ('admin2_pcode', models.CharField(max_length=20)),
                ('year', models.PositiveIntegerField()),
                ('month', models.PositiveSmallIntegerField()),
                ('events', models.PositiveIntegerField(default=0)),
                ('fatalities', models.PositiveIntegerField(default=0)),
                ('admin1', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, related_name='events', to='regions.adm1')),
            ],
            options={
                'ordering': ['year', 'month'],
                'unique_together': {('admin1', 'year', 'month', 'events', 'fatalities')},
            },
        ),
    ]
